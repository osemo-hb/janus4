...
Uses gpt-4o-mini to rerank retrieved memories by relevance.
"""
...
from janus_core.prompts import RERANK_PROMPT
...
class LLMReranker:
    ...
    async def rerank(
        ...
        try:
            ...
            prompt = RERANK_PROMPT.format(
                query=query[:settings.QUERY_TRUNCATION_LIMIT],
    ...
    def _format_memories(
        ...
        for i, item in enumerate(items):
            ...
            if len(text) > settings.RERANK_TRUNCATION_LIMIT:
                text = text[:settings.RERANK_TRUNCATION_LIMIT] + "..."
    ...
    def _parse_response(
        ...
        scores = parse_json_response(response, [], "Rerank response")
        if not scores:
            return self._passthrough(items, original_scores)
