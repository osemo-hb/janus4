...
Single LLM call that extracts entities, facts, summary, and compressed state.
"""
...
from janus_core.prompts import EXTRACTION_SYSTEM_PROMPT
...
class UnifiedExtractor:
    ...
    async def extract(self, text: str) -> ExtractionResult:
        ...
        try:
            truncated_text = text[:settings.TEXT_TRUNCATION_LIMIT]
    ...
    def _parse_response(self, response: str) -> ExtractionResult:
        ...
        empty = ExtractionResult(entities=[], facts=[], summary="")
        data = parse_json_response(response, None, "Extraction response")
        if data is None:
            return empty
        ...
        logger.debug(f"Extracted {len(entities)} entities, {len(facts)} facts")
