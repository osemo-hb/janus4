...
class ConsolidationService:
    ...
    async def consolidate(
        ...
        with tracer.start_as_current_span("consolidate") as span:
            ...
            try:
                ...
                metrics.consolidation_total.labels(
                    status="failure",
                    trigger_reason=source_episode_reason,
                ).inc()
                raise
    ...
    async def _do_consolidate(
        ...
        metrics.consolidation_duration.labels(trigger_reason=reason).observe(duration)
        metrics.consolidation_total.labels(
            status="success", trigger_reason=reason
        ).inc()
